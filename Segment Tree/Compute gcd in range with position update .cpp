...
{
    ll cnt=0;
    gcd = __gcd(a.first,b.first);
    if(a.first==gcd)cnt+=a.second;
    if(b.first==gcd)cnt+=b.second;
    return {gcd,cnt};
}
...
void build(ll arr[], ll v, ll tl, ll tr)
{
    ...
    {
        Tree[v]={arr[tl],1LL};
        //cout<<" leaf : "<<v<<" "<<Tree[v].first<<" "<<Tree[v].second<<ses;
    }
    else
    {
        ll tmid = (tl+tr)/2;
        ...
        //cout<<"Non leaf : "<<v<<" "<<Tree[v].first<<" "<<Tree[v].second<<ses;
    }
}
